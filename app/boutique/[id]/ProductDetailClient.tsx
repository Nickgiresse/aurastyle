...
export default function ProductDetailClient({
  ...
  const handleWhatsAppProduct = () => {
    const message = encodeURIComponent(
      `🛍️ *Bonjour Aura & Style !*\n\n` +
        `Je suis intéressé(e) par ce produit :\n\n` +
        `*${product.name}*\n` +
        `💰 Prix : ${formatPrice(product.price)}\n` +
        `📦 Catégorie : ${product.category || ""}\n` +
        `📏 Taille souhaitée : ${selectedSize || "À définir"}\n\n` +
        ...
        <div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className="flex-1 rounded-xl bg-[#111] py-4 font-semibold text-white transition-all duration-200 hover:bg-gray-800"
            >
              🛒 Ajouter au panier
            </button>
            <button
              onClick={handleWhatsAppProduct}
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#25D366] py-4 font-semibold text-white transition-all duration-200 hover:bg-[#1ebe5d]"
            >
